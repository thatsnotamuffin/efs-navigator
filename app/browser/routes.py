...
from flask import (Blueprint, request, abort, send_file,
                   jsonify, render_template, redirect, url_for, current_app)
from flask_login import login_required, current_user
from app.browser.utils import get_mount_base, list_directory_paginated, safe_join
...
def browse(mount_id, subpath):
    ...
    if not os.path.exists(full_path):
        current_app.logger.warning(
            '[browse] Path not found: %s (user: %s)',
            full_path, current_user.id)
        abort(404)
    ...
    try:
        ...
        current_app.logger.error(
            '[browse] 403 Forbidden – Permission denied trying to access: '
            '%s (user: %s)', full_path, current_user.id)
        abort(403)
    except (OSError, IOError) as error:
        current_app.logger.error(
            '[browse] Error listing directory %s: %s', full_path, error)
        abort(500)
...
def api_browse(mount_id, subpath):
    ...
    try:
        ...
        current_app.logger.error('[api_browse] Error: %s', error)
